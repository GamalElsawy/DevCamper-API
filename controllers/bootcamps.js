...
exports.getBootcamps = asyncHandler(async (req, res, next) => {
  res.status(201).json(res.advansedResults);
});
...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if(publishedBootcamp && req.user.role !== 'admin'){
    return next(new ErrorResponse(`user with ID ${req.user.id} has already added a bootcamp`, 400));
  }
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (!bootcamp) {
    ...
    if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin'){
      next(
        new ErrorResponse(`User with ID ${req.params.id} is not authorized to update this bootcamp`, 401)
      );
    }
...
exports.deleteBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (!bootcamp) {
    ...
    if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin'){
      next(
        new ErrorResponse(`User with ID ${req.params.id} is not authorized to delete this bootcamp`, 401)
      );
    }
    else{
    bootcamp.remove();
...
exports.getBootcampsInRadius = asyncHandler(async (req, res, next) => {
  ...
  // Earth Radius = 3963 miles / 6378 km, but we gonna use miles
